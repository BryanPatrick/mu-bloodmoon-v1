  ...
  <section class="grid gap-4">
    <header class="flex flex-wrap items-end justify-between gap-3">
      <div>
        ...
        <h2 class="mt-1 font-display text-2xl font-black uppercase text-white">Noticias, Wiki e paginas</h2>
      </div>
    ...
    <div class="grid gap-2 rounded-md border border-white/10 bg-black/20 p-3 md:grid-cols-[1fr_190px_170px]">
      ...
      <select v-model="kind" class="bm-admin-field"><option value="">Todos os tipos</option><option v-for="item in kinds" :key="item" :value="item">{{ item }}</option></select>
...
const kinds = ['NEWS', 'PAGE', 'BANNER', 'DOWNLOAD', 'NAVIGATION', 'CHARACTER', 'EQUIPMENT', 'ITEM', 'MAP', 'MONSTER', 'DROP', 'SKILL', 'EVENT', 'QUEST', 'NPC', 'GUIDE', 'LORE', 'SYSTEM']
...
const emptyForm = () => ({ id: '', title: '', slug: '', kind: 'NEWS', scope: 'SEASON_6', status: 'RAW', summary: '', content: '', seasonMin: 1 as number | null, seasonMax: 6 as number | null })
const form = reactive(emptyForm())
let timer: ReturnType<typeof setTimeout> | undefined
const load = async () => { loading.value = true; try { const result = await api.entries({ pageSize: 100, search: search.value, kind: kind.value, status: status.value }) as { data: Entry[] }; rows.value = result.data } catch { message.value = 'Falha ao carregar o conteudo.' } finally { loading.value = false } }
