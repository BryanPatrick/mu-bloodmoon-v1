...
const requestFeed = () => {
  if (['following', 'saved'].includes(feedMode.value) && !accessToken.value) return Promise.resolve({ data: [], total: 0, page: 1, pageSize: 30, totalPages: 1 })
  return api.feed({ pageSize: 30, feed: feedMode.value }, Boolean(accessToken.value))
}
...
const posts = computed(() => (feedData.value?.data || []).map(normalizePost))
...
.community-feed-tabs { display: flex; border-bottom: 1px solid var(--bm-border); }
...
@media (max-width: 899px) { .community-layout { grid-template-columns: minmax(0, 1fr); max-width: 700px; }.community-layout__right { display: none !important; } .community-mobile-ad { display: block; } }
