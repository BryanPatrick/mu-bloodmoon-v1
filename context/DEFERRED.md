...
|---|---|---|
| ~~Assigning canonical `DEC-<DOMAIN>-NNN` IDs~~ | **SUPERSEDED by Phase 10 instruction:** all 26 Hub decisions were read and indexed; their original IDs remain canonical. A readable alias, if ever useful, must remain non-canonical. | No canonical ID migration is planned; see `DECISIONS.md` and `KNOWLEDGE_HUB_MAPPING.md`. |
| Real Codex staging pilot | No real Codex session available this phase; independently schedulable per `D:\MU\hub\docs\operations\orchestration-staging.md`'s own pilot design | A real Codex session, its own staging actor/key, a fresh go-ahead. **Concrete next design, proposed here**: task it with the genuine independent content review this Context Pack's own `PRODUCT` review only lifecycle-validated (see `domains/orchestration.md`) — Codex reads `context/` fresh, checks it against the real `docs/` tree, and either approves or requests changes for real, closing the gap this pilot's `codex-staging` stand-in deliberately left open. |
