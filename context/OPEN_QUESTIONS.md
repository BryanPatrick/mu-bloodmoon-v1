...
| OQ-CTX-006 | Are Knowledge Hub decisions `cf5f14c2`/`53034c0c` (SITE_BETA_BLOCKED / NO-GO, 2026-08-08) still accurate? **Partially clarified, Phase 11**: `open-questions.md`/`open-risks.md` (preserved, read in full) independently confirm password-recovery (one cited blocker) was real and resolved by 2026-08-31, and substantial further payment work followed through Phase X — but neither preserved document mentions the *other* cited blockers (404-crash, CAPTCHA-without-rate-limit, marketplace/escrow homologation, HTTPS/TLS absence) at all, so those remain genuinely unverified either way. | Not fully resolved — real partial evidence now exists (see `REPOSITORY_KNOWLEDGE_MAP.md` §6), but not enough to close this without either a fresh audit of the remaining blockers or a Bryan decision. Per Part 21's explicit instruction, not silently resolved to either side. |
| OQ-CTX-007 | Three real decision domains — `community`, `security`/`privacy`, `testing` — have no matching `context/domains/*.md` stub among the current 12. `ADR_INDEX.md`'s domain gaps (accounts, product-direction, security) reconfirm the same pattern from a second, independent angle (the ADRs, not just the Hub decisions). | Still open — adding new domain stubs wasn't authorized by either phase's brief. |
