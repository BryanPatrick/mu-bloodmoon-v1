...
const newClaims = [
	...
	C({
		claimId: 'CLAIM-133',
		statement: 'The committed Worker (apps/game-data-worker/src/commands.ts, identical blob on all 30 local branch tips) accepts only CREATE_GAME_ACCOUNT and its D1 schema.sql still has CHECK (command_type = \'CREATE_GAME_ACCOUNT\'); the Portal client types, the Agent handlers and the D1 migration 0004 exist for GRANT_VIP, SYNC_VIP_TIER, ANONYMIZE_GAME_ACCOUNT and PURGE_GAME_ACCOUNT, but the Worker routing for them exists only as uncommitted changes in the mu-bloodmoon-v1-openbeta worktree -- so those four commands cannot flow end to end from committed code.',
		sourceId: CODE,
		sourceAuthority: 'CURRENT_CODE',
		sourceLocation: 'apps/game-data-worker/src/commands.ts:17,219-230; apps/game-data-worker/db/schema.sql:80; apps/game-data-worker/db/migrations/0004_gamebridge_extension_commands.sql; git hash-object comparison across local branch tips; mu-bloodmoon-v1-openbeta working tree (git diff --stat: commands.ts +189, schema.sql, commands.spec.ts)',
...
const doneItems = [
	...
	['CLAIM-133', 'HIGH', 'file_read', 'Worker commands.ts / schema.sql on committed branches vs the openbeta working tree', 'CONFIRMED_BY_CONFIG -- committed blob accepts CREATE_GAME_ACCOUNT only on all 30 local branch tips; extension only uncommitted in mu-bloodmoon-v1-openbeta.', ['apps/game-data-worker/src/commands.ts']],
