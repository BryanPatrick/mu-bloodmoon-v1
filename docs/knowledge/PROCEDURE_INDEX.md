...
| Inspect the legacy DMN CMS's live schema | ADMIN/DATABASE | `docs/economy/legacy-dmn-cms-and-currency-investigation.md` | Same | SQL Server (production, dormant tables) | MEDIUM (production DB, read-only via `bm-sql.cmd`) | Read-only: none. Any DROP/decommission: YES, explicit | SOURCE_ONLY (no decommission runbook needed — `KEEP_DORMANT` decided 2026-09-18, GAP-P18-05) |
...
| Reload the Buy Vip settings (`GameServerInfo - Command.dat`) or a `CustomBuyVipAndCoin` entry (`Data/Custom`) after editing — those two config families specifically | GAMESERVER | This row (18C Part 20, corrected 18D) — vendor-demonstrated procedure: open GameServer Startup GUI → double-click the server row → "Reloads" tab → pick the matching reload ("Reload Comand" for `GameServerInfo - Command.dat`, "Reload Custom" + "Reload Shop" for `Data/Custom`/shop-adjacent files) → change shown taking effect with no restart. **Vendor-demonstrated once per config family, not tested on Blood Moon's own instance** — and both features are currently disabled/inert on Blood Moon (CLAIM-101, CLAIM-118), so this only matters if one is ever enabled | `CASH_VIP_INTEGRATION_MAP.md` Part 1 and machine claims CLAIM-110 (`gqtSk1pdti4`, Command) and CLAIM-117 (`XUeN6U74zME`, Custom+Shop) — ~~2 independent demonstrations agreeing~~ **(18D correction)**: two *different* families, one video each, not the same procedure confirmed twice | GameServer | LOW (read/reload only, vendor-demonstrated, not a config edit itself) | None for the reload action itself; editing the underlying config file follows normal change-review practice | RUNBOOK_READY — narrow scope, does **not** extend to `CustomXShop.txt`/`CashShopProduct.txt` (still `KNOWLEDGE_NO_RUNBOOK`, different files, unproven) |
...
| Decide the legacy DMN CMS's fate (keep/decommission/audit) | ADMINISTRATION | `docs/economy/legacy-dmn-cms-and-currency-investigation.md`; decision recorded in `LEGACY_SUPPLIER_INDEX.md` | Same | SQL Server (production) | Decision-only, no code risk | This WAS the approval step — **decided by Bryan 2026-09-18: `KEEP_DORMANT`** (do not delete/decommission; dependency audit incomplete). Reopening it needs a new explicit decision | DECIDED — no runbook required while `KEEP_DORMANT` stands |
...
| Classification | Count (Phase 18) | Count (after Phase 18C) | Count (after Phase 18D) |
|---|---|---|---|
| RUNBOOK_READY | 8 | 9 (+1: GameServer Command/Custom reload) | 9 (row corrected, not added) |
| KNOWLEDGE_NO_RUNBOOK | 1 | 1 (narrowed to X-Shop/CashShop.txt specifically) | 1 |
| SOURCE_ONLY | 2 | 2 | 1 (the DmN schema inspection row) |
| DECIDED — no runbook required | — | — | 1 (DmN fate: `KEEP_DORMANT`, Bryan 2026-09-18) |
| KNOWLEDGE_MISSING | 2 | 2 (item-delivery trace and DmN decommission procedure both lacked evidence) | 2 (item-delivery trace still missing; the DmN decommission procedure is now moot while `KEEP_DORMANT` stands, but still unwritten — see `KNOWLEDGE_GAPS.md`) |
