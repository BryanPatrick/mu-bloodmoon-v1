...
| 6 | Personal shop / market (`CustomMarketShop`) | `SellerAccount`/`SellerName` (real PK: `ItemGUID`) | **BLOCK** (if any row) — **confirmed 2026-08-30**: the table has no status/active/sold/cancelled column at all (`ItemGUID`/`AuthCode`/`SellerAccount`/`SellerName`/`Price`/`PriceType`/`Item`/`Tax` is the complete real column list, verified via `sys.columns`) — "row exists" is the only signal this schema offers, so blocking on any existing row is the confirmed-correct behavior, not a conservative placeholder. | An active personal-shop listing (real Zen/items at stake, another player mid-purchase) cannot be silently anonymized out from under a live trade — block until no listing row exists, mirroring the Portal's own marketplace-listing block (Phase 14 Part D). |
...
| 10 | Friends (`T_FriendMain`, `T_FriendList`, `T_WaitFriend`, `T_FriendMail`) | **Confirmed 2026-08-30** via `sys.columns`/`sys.indexes`: `T_FriendMain` PK is `GUID` (also has `Name`); `T_FriendList`/`T_WaitFriend` have **no `Name` column at all** and **no PK/unique index** — their real key is the numeric `GUID`/`FriendGuid` pair, bridged from a character name only via `T_FriendMain.Name→GUID`. This corrected a real bug: the procedures originally assumed a `Name` column on `T_FriendList`/`T_WaitFriend`, which would have failed to compile against production (`Invalid column name`). `T_FriendMail` (correspondence, not membership) confirmed real but untouched by either procedure. | **DETACH** | Remove this account's own friend-list rows via the confirmed `GUID`-based lookup (see the procedures' own headers). Other players' rows referencing this character by `FriendName` become stale references pointing at a now-renamed character — **UNKNOWN_DISPLAY_BEHAVIOR** (does the client show "player not found" gracefully, or error?) — still flagged, not assumed benign; this is a client-behavior question the schema check doesn't answer. |
...
- **`GAME_BRIDGE_GRANT_VIP_ENABLED`** *(Phase 20B note: documentation name; the code option is `AgentOptions.GrantVipEnabled`, environment variable `BLOODMOON_AGENT_Agent__GrantVipEnabled`)* — checked by `GameCommandProcessor` before dispatching a `GRANT_VIP` command; if false, the Agent reports `FAILED_FINAL COMMAND_TYPE_DISABLED` (a new, distinct code from `COMMAND_TYPE_DENIED`, so an operator can tell "we don't support this at all" apart from "we support it but it's paused") without ever calling `IGameDatabaseWriter`.
...
**Artefatos já produzidos nesta rodada** (todos "reviewed, not executed" — nada rodou contra um SQL Server real, local ou de produção, pelo bloqueio documentado em `docs/environment/sql-server-test-environment.md`):
- Worker (`apps/game-data-worker`): migração D1 + `commands.ts` estendidos para os 5 tipos de comando, com testes reais (`@cloudflare/vitest-pool-workers`, D1 local) — **implementado e testado localmente**.
- Agent (`apps/game-bridge-agent`, .NET): os quatro novos comandos, kill switches, lock namespace compartilhado, testes xUnit reais — **implementado e testado localmente**.
- As quatro stored procedures (`dbo.bm_GrantVip`, `dbo.bm_SyncVipTier`, `dbo.bm_AnonymizeGameAccount`, `dbo.bm_PurgeGameAccount`) e o script de grants — texto T-SQL completo, revisado, **nunca executado**: `references/game-data/sql-discovery/gamebridge-extension-20260830/`. `bm_AnonymizeGameAccount` carrega dois pontos marcados `VERIFY_BEFORE_USE` no próprio cabeçalho do arquivo (esquema de nome-tombstone; a técnica `NOCHECK`/`CHECK CHECK CONSTRAINT` para renomear `Character`/`CustomQuest` em lockstep) que precisam de confirmação contra um engine real antes de instalação.

---

## Nota relacionada — Fase T (2026-09-02), catálogo legado X-Shop/CashShop

`LegacyCatalogConfigService#sync()` (a camada de controle do catálogo
legado X-Shop/CashShop, ver `docs/decisions/0024-legacy-shop-control-plane.md`)
usa nomes de comando conceituais no mesmo espírito deste plano
(`SET_XSHOP_PRODUCT_ENABLED`/`PRICE`/`CURRENCY` etc.) mas **não reutiliza
nem estende o pipeline Worker/Queue/Agent acima** — o método é hoje só um
guard (permissão + feature flag + `NOT_IMPLEMENTED_PENDING_RUNTIME_KNOWLEDGE`),
sem nenhum comando real despachado. Se uma sincronização real for
construída no futuro, este pipeline (D1 `game_command`, Agent
`GameCommandWorker`, HMAC, idempotência via `ProvisioningLedger`) é o
candidato natural a reutilizar em vez de inventar um segundo transporte
— mas isso não foi decidido nem começado nesta fase; os campos de
config X-Shop/CashShop também têm uma lacuna própria que os quatro
comandos deste documento não têm (`RUNTIME_MUTABLE`/`RELOAD_REQUIRED`/
`RESTART_REQUIRED` = `UNKNOWN` para todo campo, ver
`docs/economy/xshop-cashshop-config-field-matrix.md` e OQ-031).
