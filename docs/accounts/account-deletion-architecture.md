...
Both follow the exact job-queue shape already proven for `GRANT_VIP` in Part C (`GameBridgeJob` row, `idempotencyKey`-guarded, claimed atomically, retried with backoff) — no new queue mechanism was invented.

**Updated, 2026-08-30 (GameBridge extension round)**: both operations now have a real sender, `AccountLifecycleBridgeService` (`apps/api/src/modules/accounts/account-lifecycle-bridge.service.ts`) — see `docs/gamebridge/gamebridge-agent-extension-plan.md` and `docs/gamebridge/gamebridge-local-testing.md` for the full design and test record. It dispatches the queued job via the real `GameCommandTransportClient` (the same transport `CREATE_GAME_ACCOUNT`/`GRANT_VIP` use), polls for a terminal result, and mirrors both ends into `AuditEvent` (`gamebridge.anonymize.sent`/`.completed`/`.failed`, same shape for `purge`), correlated via the command's own `commandId`. Gated behind `ACCOUNT_LIFECYCLE_BRIDGE_ENABLED` (default off) at the Portal layer and the Agent's own `GAME_BRIDGE_ANONYMIZE_ENABLED`/`GAME_BRIDGE_PURGE_ENABLED` kill switches (also default off) — enabling the Portal-side sender alone does nothing on a real GameServer until the Agent-side switch is separately flipped. The four stored procedures the Agent calls (`dbo.bm_AnonymizeGameAccount`, `dbo.bm_PurgeGameAccount`, and the two VIP ones) remain **reviewed but never executed** against any SQL Server, pending the environment blocker in `docs/environment/sql-server-test-environment.md`.
...
9. `ANONYMIZE_GAME_ACCOUNT`/`PURGE_GAME_ACCOUNT` `GameBridgeJob` rows are queued for any account with a `GameAccountIdentity`. As of the 2026-08-30 GameBridge extension round, `AccountLifecycleBridgeService` (see above) is a real sender for these — real GameServer-side execution still requires the Agent's own kill switch and, ultimately, the stored procedures actually being installed on a real SQL Server (still blocked, see `docs/environment/sql-server-test-environment.md`).
...
## Phase 15 addendum — self-service request flow, GameBridge senders, exit feedback, RBAC narrowing (2026-08-30)
...
### Exit feedback questionnaire (new this round)

`POST account/deletion/request` accepts an optional `feedback: { reasons: [{code, detail?}], otherText? }` body. Reason codes: `LACK_OF_TIME`, `PROGRESSION`, `BALANCING`, `BUGS`, `PERFORMANCE_CONNECTION`, `PLAYER_COMMUNITY_ISSUE`, `STAFF_SUPPORT_ISSUE`, `SHOP_VIP_ECONOMY`, `ANOTHER_SERVER`, `ACCOUNT_ISSUE`, `PRIVACY_SECURITY`, `OTHER` (see `account-deletion.contract.ts`'s `EXIT_FEEDBACK_REASON_CODES`). `detail` is free text per reason, standing in for whatever contextual sub-question the frontend shows for that code — the backend does not encode a rigid per-reason sub-question schema, since that's a UI concern. **Never blocks the deletion request**: an unrecognized reason code is silently dropped (not rejected), and any failure writing the feedback row is swallowed — the request itself always proceeds regardless. Persisted in a new `AccountDeletionFeedback` table, deliberately **not** a relation field on `Account` (same no-hard-FK precedent as `AccountDeletionRequest`) so it can outlive the account: `executeNormalDeletion()` unlinks `accountId` (sets it `null`, stamps `anonymizedAt`) rather than deleting or cascading the row, so the structured reasons/free text survive for product analytics after the account itself is gone. Structurally separate from `AccountModeration`/`AuditEvent` (security/antifraud data) — never joined or merged with those for analytics purposes.

**UPDATE, 2026-08-30 (same-day follow-up)**: ANTERIOR — this section originally said the questionnaire UI, contextual offramps, and analytics were all deferred. NOVA — Bryan asked for the frontend foundation and analytics to be built in the same round. Both are now real:

- **Frontend**: `apps/web/pages/painel/privacidade.vue` (route `/painel/privacidade`, linked from the player nav in `ManagementShell.vue`) implements the full "Privacidade e meus dados" flow — view/export data (with a "corrigir informações"/"gerenciar consentimentos" section explicitly marked `[EM BREVE]`, never advertised as working), the exit questionnaire (checkbox list of the 12 reason codes + free text, never blocking, with contextual retention offers for `BUGS`/`STAFF_SUPPORT_ISSUE`/`ACCOUNT_ISSUE`/`LACK_OF_TIME` that always coexist with a "Continuar com a exclusão" button), the four required unchecked-by-default acknowledgement checkboxes, and status display mapped from the real backend `AccountDeletionRequestStatus` values (`NONE`/`REQUESTED`/`CONFIRMED`/`CANCELLED`/`EXECUTED`) with a cancel action during the grace period. Backed by a new `usePrivacyApi.ts` composable, mirroring `useAccountSecurityApi.ts`'s existing fetch/auth pattern exactly. Verified via the dev server: no Vue/TypeScript compile errors, the route resolves and correctly redirects to `/login` when unauthenticated (no fatal error), no console errors. **Not verified**: full authenticated rendering against a live `apps/api` instance (would require standing up the whole backend stack; not done this round).
- **Analytics**: `AccountDeletionRequestService.exitFeedbackSummary()` (new) aggregates `AccountDeletionFeedback` rows by reason code, current window vs. prior window of the same length, reading only `reasons`/`submittedAt` — never `accountId`, so it works identically before and after a row's account link is cleared. Exposed via `GET /admin/accounts/deletion/exit-feedback/summary`. Tested (`EXIT_FEEDBACK_SUMMARY_COUNTS_BY_REASON_WITHOUT_READING_IDENTITY`).

**Still not built**: contextual sub-questions specific to each reason code beyond the single free-text field (the backend's `detail` field per reason supports this; the frontend doesn't yet ask type-specific follow-ups), and the "corrigir informações"/"gerenciar consentimentos" operations themselves (explicitly marked planned, not silently missing).

### PURGE RBAC narrowed to SUPER_ADMIN by default (new this round)

`pre-beta-purge/*` endpoints on `AccountDeletionController` now require `admin.accounts.purge.manage` — a **new, separate** permission from `admin.accounts.status.manage` (which `normal/*` still uses). Neither permission is part of `ADMIN`'s default role permission set (`permissions.ts`'s `rolePermissions.ADMIN` is a small curated list, not a blanket `admin.*` grant — confirmed by reading the file directly, not assumed) — every `admin.*` capability, including this new one, requires an explicit `SUPER_ADMIN`-granted `AccountPermission` override before an `ADMIN` account can use it. This was a real, pre-existing gap: before this round, `pre-beta-purge` shared `admin.accounts.status.manage` with ordinary account-status management, so any admin delegated that common capability also received irreversible purge access as an unintended side effect. Two new observability endpoints were added alongside the fix: `GET pre-beta-purge/:betaCycleId/report` (per-account GameBridge delivery status for a purge batch — the "relatório do batch" / post-verification Bryan asked for) and `GET lifecycle-bridge` (general ANONYMIZE/PURGE job status needing attention).

### Privacy center compatibility (architecture note, not built this round)

Bryan asked that the deletion flow be architected compatibly with a future consolidated "Privacidade e meus dados" area (view my data, request a copy, correct data, manage consents, request data deletion, delete account, track requests) without necessarily building that full area now. The self-service flow above is already shaped compatibly: `GET account/deletion/status` (track requests), `GET account/deletion/export` (view/copy my data, already built Phase 15), `POST account/deletion/request`/`confirm`/`cancel` (delete account) are all independent, narrowly-scoped endpoints under `account/deletion/*` rather than one monolithic "delete" action — a future `account/privacy/*` surface could sit alongside these and reuse `AccountDeletionRequestService`'s existing methods directly, with no restructuring needed. **Not built this round**: consent management, data correction, and the consolidated frontend area itself — flagged as real, deferred scope, not silently dropped.
