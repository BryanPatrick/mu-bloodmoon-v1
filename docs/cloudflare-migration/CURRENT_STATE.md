...
| `CONTAINER_STORAGE_READY` | `CODE_READY = YES` as of Phase CF-R2-03 (every domain has a working local/R2 `StorageProvider` switch, zero raw unabstracted `node:fs` write paths remain in `apps/api/src`); `PRODUCTION_ACTIVATED = NO` (no `*_STORAGE_PROVIDER` is `r2` anywhere real, `RISKS.md` CF-R1) | `docs/cloudflare-migration/R2_ASSETS.md`'s CF-R2-03 section |
