...
| NestJS API | cPanel Node.js Selector (`/home/mubloodxz/bmapi`) | Node (LSAPI) | none directly (talks to MySQL) | MySQL, SMTP, Turnstile siteverify, Asaas, Mercado Pago, Cloudflare D1/Queue (Game Data client) | LOW_TO_MEDIUM for the chosen Container path (near-zero application change); HIGH if the future native-Workers path is ever pursued — see `API_MIGRATION.md` | CRITICAL (auth, payments, marketplace, game accounts) | **Cloudflare Containers** (decided 2026-09-22, `DECISIONS.md`) — native Workers stays a possible future optimization, not this migration's target | Container proof (`CF-API-02`) must pass before any real cutover; database exit (Phase 5) blocks a *production* cutover, but the Container proof itself can run against a disposable database first |
| MySQL | cPanel host, bound to `127.0.0.1` | MySQL 8.x (cPanel-managed) | all portal/financial/community/marketplace data | none outbound | HIGH — the actual constrained resource of this whole program | CRITICAL (every financial/portal write) | Bryan-controlled or managed MySQL-compatible (vendor **not chosen**, see `DATABASE_MIGRATION.md`; D1 explicitly excluded) | `CF-DB-01` (candidate evaluation + the Phase 17R backup-restore P1); blocks any *production* API cutover regardless of runtime |
