...
| 4 | API runtime: Containers (chosen initial target) | apps/api runtime | `API_MIGRATION.md` compatibility findings for both options | **DECIDED 2026-09-22**: `API_INITIAL_MIGRATION_TARGET = CLOUDFLARE_CONTAINERS`, `API_WORKERS_NATIVE = FUTURE_OPTIMIZATION` (`DECISIONS.md`). **Update, `CF-EXIT-01` (2026-09-23)**: the container proof (`CF-API-02R`) now appears to have actually run, with real evidence, on a concurrent unmerged Codex branch (`RISKS.md` CF-R4, `PROVIDER_EXIT_CHECKLIST.md`) — consumed read-only, not this program's own verified result, not merged | this was a decision phase for *which runtime*; the build/proof work now has real evidence on an unmerged branch — a review/adoption decision, not a from-scratch proof, is what's still outstanding |
...
| 7 | DNS/domain cutover | authoritative DNS | Bryan has confirmed sufficient registrar/DNS control (`DNS_AND_DOMAIN.md` — currently `PENDING_TRANSFER`; full inventory + a documented domain-control gap done `CF-DNS-01`, 2026-09-23 — registrant confirmed as Bryan, but registro.br/DNS-zone/nameserver-level *access* still unconfirmed); target Cloudflare DNS record set designed (`DNS_AND_DOMAIN.md`), including the previously-undocumented `update.mubloodmoon.com.br` launcher-update property and full mail (MX/SPF/DKIM/DMARC) preservation requirements. **Mail dependency audit now complete** (`CF-MAIL-01`, 2026-09-23, `EMAIL_MIGRATION.md`): 3 application email consumers on one centralized SMTP transport, `MAILBOX_INVENTORY = UNKNOWN`, 5-candidate transactional-provider shortlist researched — **no provider chosen, mail migration is its own separate, independently-timed decision, not a Phase 7 prerequisite** (SPF/DKIM for a new transactional provider can be added at the current DNS host before Phase 7, per `EMAIL_MIGRATION.md` §9) | production hostnames resolve through Cloudflare, TLS re-verified end to end, **and mail delivery re-verified end to end** (a real, easy-to-silently-break dependency this phase found — MX points at the bare domain, not a distinct mail host) | revert nameservers at the registrar; this is the highest-blast-radius single step in the program and gets its own dedicated runbook before it is attempted |
| 8 | Remove current provider completely | everything remaining | Phases 1–7 all complete and stable for an agreed observation period; `BACKUP_EXIT_READY = YES` (`BACKUP_STRATEGY.md`, `CF-BACKUP-01` — architecture designed and restore chain re-proven, real off-host copy + encryption still outstanding) | `CURRENT_PROVIDER = ZERO`: no service, DNS record, or credential still depends on the old host | not reversible in the same sense — this is the program's actual finish line, approached last and only with explicit sign-off |
...
1. **`CF-API-02R` — Container POC.** Run the prepared container proof
   ...
   MySQL/MariaDB, never production. **Update, `CF-EXIT-01`
   (2026-09-23)**: real evidence this proof has run now exists on a
   concurrent, unmerged Codex branch
   (`infra/cloudflare-api-container-poc`) — see `RISKS.md` CF-R4,
   `PROVIDER_EXIT_CHECKLIST.md`. Consumed read-only; not merged; a
   review/adoption decision is the remaining step, not a from-scratch
   re-run.
